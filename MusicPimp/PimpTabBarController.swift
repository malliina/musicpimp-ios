...
class PimpTabBarController: UITabBarController {
  ...
  let flippablePlayer: UIViewController = TabUtils.shared.attachTab(
    vc: PlayerParent(persistent: true), title: "Player", fontAwesomeName: "play-circle")
  let stackedPlayer: UIViewController = TabUtils.shared.attachTab(
    vc: SideBySidePlayer(persistent: true), title: "Player", fontAwesomeName: "play-circle")
  // most popular and recent
  let flippableTopList: UIViewController = TabUtils.shared.attachTab(
  ...
  override func viewDidLoad() {
    ...
    var permanentTabs = [
      UINavigationController(
        rootViewController: utils.attachTab(
          vc: LibraryContainer(), title: "Music", fontAwesomeName: "music")),
      UINavigationController(rootViewController: playerFor(traits: trait)),
      UINavigationController(
        rootViewController: utils.attachTab(
          vc: PlaybackContainer(
  ...
  override func willTransition(
    ...
    if let vcs = viewControllers, vcs.count > 2 {
        ...
        let listNav = vcs[2] as? UINavigationController
      {
        ...
        listNav.setNavigationBarHidden(
          newCollection.horizontalSizeClass == .regular, animated: true)
      }
    }
...
class TabUtils {
  ...
  func icon(_ name: String, selected: Bool) -> UIImage {
    ...
    let image = UIImage(
      icon: name, backgroundColor: .clear, iconColor: iconColor, fontSize: tabIconFontSize)
