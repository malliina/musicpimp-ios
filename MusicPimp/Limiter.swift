...
class Limiter: CustomStringConvertible {
  ...
  func isWithinLimit() -> Bool {
    ...
    return PimpSettings.sharedInstance.isUserPremium || runs.count < welcomeLimit
      || runsWithinDuration < limit
  }
  ...
  var description: String {
    "\(limit) tracks every \(duration) with \(welcomeLimit) welcome tracks"
  }
}
