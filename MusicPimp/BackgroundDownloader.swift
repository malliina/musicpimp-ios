...
public class DownloadProgressUpdate {
  ...
  public var destinationURL: URL { info.destinationURL }

  var isComplete: Bool? { written == totalExpected }

  init(
    info: DownloadTask, writtenDelta: StorageSize, written: StorageSize, totalExpected: StorageSize?
  ) {
    self.info = info
...
class BackgroundDownloader: NSObject, URLSessionDownloadDelegate, URLSessionTaskDelegate,
  URLSessionDelegate
{
  let log = LoggerFactory.shared.network(BackgroundDownloader.self)
  typealias TaskID = Int

  static let musicDownloader = BackgroundDownloader(
  ...
  init(basePath: String, sessionID: SessionID) {
    ...
    self.tasks = Dictionary(
      uniqueKeysWithValues: PimpSettings.sharedInstance.tasks(sessionID).tasks.map { t in
        (t.taskId, t)
      })
