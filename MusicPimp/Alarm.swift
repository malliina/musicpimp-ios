...
extension AlarmJson where T == AlarmJob {
  func asAlarm() -> Alarm { Alarm(id: id, track: job.track, when: when, enabled: enabled) }
}
...
class Alarm {
  ...
  func asJson() -> AlarmJson<AlarmJobIdOnly> {
    AlarmJson(id: id, job: AlarmJobIdOnly(track: track.id), when: when, enabled: enabled)
  }
}
...
struct AlarmTime: Codable {
  ...
  var time: ClockTime { ClockTime(hour: hour, minute: minute) }
}
...
class MutableAlarmTime {
  ...
  func toImmutable() -> AlarmTime {
    AlarmTime(hour: hour, minute: minute, days: days)
  }
}
