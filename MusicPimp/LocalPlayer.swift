...
class LocalPlayer: BasePlayer, PlayerType {
  ...
  private func buildUrl(track: Track) -> URL {
    ...
    let authQuery = LibraryManager.sharedInstance.libraryUpdated.authQuery
  ...
  fileprivate func initAndPlay(_ track: Track) -> ErrorMessage? {
    ...
    playerItem.addObserver(
      self, forKeyPath: LocalPlayer.statusKeyPath, options: .initial,
      context: &LocalPlayer.itemStatusContext)
    ...
    p.addObserver(
      self, forKeyPath: LocalPlayer.statusKeyPath, options: .initial,
      context: &LocalPlayer.playerStatusContext)
  ...
  func closePlayer() {
    if let player = player {
      if let item = player.currentItem {
        item.removeObserver(
          self, forKeyPath: LocalPlayer.statusKeyPath, context: &LocalPlayer.itemStatusContext)
      }
      player.removeObserver(
        self, forKeyPath: LocalPlayer.statusKeyPath, context: &LocalPlayer.playerStatusContext)
      ...
      notificationCenter.removeObserver(
        self, name: NSNotification.Name.AVPlayerItemFailedToPlayToEndTime,
