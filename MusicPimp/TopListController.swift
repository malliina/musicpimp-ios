...
class TopListController<T: TopEntry>: BaseMusicController, LibraryDelegate {
  ...
  var maybeFeedback: String? {
    return self.tracks.count == 0 ? (hasLoaded ? self.emptyMessage : self.loadingMessage) : nil
  }
  ...
  private func onMore(_ from: Int, newRows: Int, expectedSize: Int) {
    ...
    if (from + newRows) == expectedSize {
      ...
      self.log.info(
        "Updated table with \(indexPaths.count) more items from \(from) to \(from+newRows-1)")
    }
  }
