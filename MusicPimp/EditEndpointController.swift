...
extension Selector {
  fileprivate static let saveClicked = #selector(EditEndpointController.onSave(_:))
  fileprivate static let cancelClicked = #selector(EditEndpointController.onCancel(_:))
  fileprivate static let serverTypeChanged = #selector(EditEndpointController.serverTypeChanged(_:))
  fileprivate static let testClicked = #selector(EditEndpointController.testClicked(_:))
}
...
class EditEndpointController: PimpViewController {
  ...
  func initUI() {
    ...
    let views: [UIView] = [
      typeControl, cloudIDLabel, cloudIDField, nameLabel, nameField, addressLabel, addressField,
      portLabel, portField, usernameLabel, usernameField, passwordLabel, passwordField,
      protocolControl, activateLabel, activateSwitch, testButton, feedbackText,
    ]
    views.forEach { (v) in
  ...
  fileprivate func adjustVisibility(_ serverType: ServerType) {
    ...
    let nonCloudViews: [UIView] = [
      nameLabel, nameField, addressLabel, addressField, portLabel, portField, protocolControl,
    ]
  ...
  func parseEndpoint() -> Endpoint? {
    ...
    if let serverType = readServerType(typeControl) {
      if serverType == ServerType.cloud {
        ...
        return Endpoint.cloud(
          id: id, cloudID: cloudIDField.text!, username: usernameField.text!,
        ...
        if let portText = portField.text, let port = Int(portText) {
          ...
          let existsEmpty = [nameField, addressField, portField, usernameField, passwordField]
            .exists({ $0.text!.isEmpty })
          if existsEmpty {
            return nil
          }
          return Endpoint(
            id: id, serverType: serverType, name: nameField.text!, ssl: ssl,
            address: addressField.text!, port: port, username: usernameField.text!,
            password: passwordField.text!)
        }
      }
    }
