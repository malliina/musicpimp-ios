...
class BaseTableController: UITableViewController {
  ...
  func feedbackLabel(_ text: String) -> UILabel {
    ...
    let frame = CGRect(
      x: 0, y: 0, width: self.view.bounds.size.width, height: self.view.bounds.size.height)
...
class IAPConstants {
  ...
  static let Message =
    "The free version of this app lets you play a couple of tracks per day. A one-time purchase unlocks MusicPimp Premium which enables unlimited playback."
...
extension UIViewController {
  ...
  func suggestPremium() {
    let sheet = UIAlertController(
      title: IAPConstants.Title, message: IAPConstants.Message,
      preferredStyle: UIAlertController.Style.alert)
    let premiumAction = UIAlertAction(
      title: IAPConstants.OkText, style: UIAlertAction.Style.default
    ) { a -> Void in
      self.navigationController?.pushViewController(IAPViewController(), animated: true)
    }
    let notInterestedAction = UIAlertAction(
      title: IAPConstants.CancelText, style: UIAlertAction.Style.cancel
    ) { a -> Void in
    }
