...
extension Selector {
  ...
  fileprivate static let seekChanged = #selector(PlayerController.sliderChanged(_:))
}
...
class PlayerController: ListeningController, PlaybackDelegate {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    let img = UIImage(
      icon: "fa-volume-up", backgroundColor: UIColor.clear, iconColor: UIColor.blue, fontSize: 24)
    navigationItem.rightBarButtonItems = [
      UIBarButtonItem(image: img, style: .plain, target: self, action: .volumeClicked)
    ]
    ...
    if let thumbImage = PlayerController.seekThumbImage {
      seek.setThumbImage(
        imageWithSize(image: thumbImage, scaledToSize: CGSize(width: 14, height: 14)), for: .normal)
    }
  }
  ...
  func initUI() {
    addSubviews(views: [
      seek, positionLabel, durationLabel, artistLabel, albumLabel, titleLabel, coverContainer,
    ])
