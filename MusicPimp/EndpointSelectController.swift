...
extension Selector {
  fileprivate static let addClicked = #selector(EndpointSelectController.onAddNew(_:))
}
...
class EndpointSelectController: BaseTableController, EditEndpointDelegate {
  ...
  func endpointRowAction(_ tableView: UITableView, title: String, f: @escaping (Int) -> Void)
    -> UITableViewRowAction
  {
